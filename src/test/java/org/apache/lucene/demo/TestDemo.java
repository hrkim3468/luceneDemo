...
public class TestDemo {
	...
	private void testOneSearch(File indexPath, String query, int expectedHitCount) throws Exception {
		...
		try {
			...
			output = bytes.toString(Charset.defaultCharset().name()); 
		
		} catch (Exception e) {
			e.printStackTrace();
		}
		
		System.out.println("output=" + output + "=>" + output.contains(expectedHitCount + " total matching documents"));		
	...
	public void testIndexSearch() {
		...
		File dir = new File("C:\\tmp_index2\\docs");
		File indexDir = new File("C:\\tmp_index2\\idx");
